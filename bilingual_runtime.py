...
def install(namespace: dict[str, Any]) -> None:
    ...
    def live_payload(config: Any) -> dict[str, Any]:
        ...
        news_by_symbol = client.news_for_symbols(stocks, config.report_date - timedelta(days=92), config.report_date)
        industries = list(dict.fromkeys(str(stock.get("industry") or "") for stock in stocks.values() if stock.get("industry")))
        ...
        for position, (symbol, stock) in enumerate(stocks.items(), start=1):
            ...
            stock["news"] = bilingual_news(news_by_symbol.get(symbol, []), config.report_date, translator, limit=5)
        translator.flush()
    ...
    def write_outputs(config: Any, payload: dict[str, Any]) -> None:
        ...
        sources = [source for source in list(metadata.get("sources") or []) if "translation" not in str(source).lower() and "argos" not in str(source).lower()]
